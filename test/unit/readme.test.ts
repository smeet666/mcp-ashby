...
/** The prefix an environment variable would carry, were there one to carry it. */
const ENV_PREFIX = "ASHBY_";
...
describe("the settings", () => {
  it("announce nothing, because the server reads no environment variable", () => {
    const sources = execSync("git ls-files src", { cwd: ROOT, encoding: "utf8" })
      .split("\n")
      .filter((path) => path.endsWith(".ts"));
    const declared = sources.filter((path) =>
      readFileSync(join(ROOT, path), "utf8").includes(ENV_PREFIX),
    );

    expect(declared).toEqual([]);
    for (const half of [english, french]) {
      const section = half.slice(half.indexOf("## Configuration"));
      expect(tableRows(section, "Variable")).toEqual([]);
    }
  });
});
